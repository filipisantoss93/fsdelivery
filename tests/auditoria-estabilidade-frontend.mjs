...
const customerAccess=read('js/customer-order-access.js');
const publicStore=read('js/loja-operacional.js');
...
assert(!/transitionend|setTimeout\(enhanceSuccess/.test(postOrder),'Vínculo pós-checkout não pode depender de animação ou atraso arbitrário.');
assert(/fs:public-order-completed[\s\S]{0,180}claimCompletedOrder/.test(postOrder),'Evento de pedido concluído deve acionar o vínculo diretamente.');
...
assert(/recuperar_pedido_dispositivo/.test(customerHistory),'Histórico deve oferecer recuperação protegida por pedido.');
assert(/raw\.length===12\|\|raw\.length===13/.test(customerAccess),'Normalização compartilhada deve reconhecer DDI brasileiro.');
assert(/normalizePhone\?\.\(raw\)[\s\S]{0,80}slice\(0,11\)/.test(publicStore),'Campo público deve remover +55 antes de limitar o telefone local.');
assert(/phone=normalizePhone\(formData\.get\('phone'\)\)/.test(publicCheckout),'Checkout deve enviar o WhatsApp canônico.');
assert(/replace\(\/\\\.html\$\/i,''\)/.test(customers),'Endereços salvos devem funcionar também em URLs limpas.');
assert(!/innerHTML/.test(customerAccess),'Contrato de credenciais não deve manipular a interface.');
