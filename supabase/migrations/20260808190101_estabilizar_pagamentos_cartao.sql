-- Estabiliza recorrência, ambientes Efí e aplicação idempotente de webhooks de pedidos.

alter table public.planos_assinatura
  add column if not exists efi_plan_ambiente text;

alter table public.assinaturas
  add column if not exists efi_ambiente text;

alter table public.planos_assinatura
  drop constraint if exists planos_assinatura_efi_plan_ambiente_check;

alter table public.planos_assinatura
  add constraint planos_assinatura_efi_plan_ambiente_check
  check (efi_plan_ambiente is null or efi_plan_ambiente in ('homologacao', 'producao'));

alter table public.assinaturas
  drop constraint if exists assinaturas_efi_ambiente_check;

alter table public.assinaturas
  add constraint assinaturas_efi_ambiente_check
  check (efi_ambiente is null or efi_ambiente in ('homologacao', 'producao'));

create unique index if not exists assinaturas_recorrencia_cartao_ativa_uidx
  on public.assinaturas (usuario_id)
  where meio_pagamento = 'cartao'
    and renovacao_automatica
