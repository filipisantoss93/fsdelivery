(()=>{
  function setupCardValidation(){
    const customerSection=document.getElementById('card-customer-section');
    if(!customerSection)return;
    const sync=()=>customerSection.querySelectorAll('input').forEach(input=>{input.disabled=customerSection.hidden});
    new MutationObserver(sync).observe(customerSection,{attributes:true,attributeFilter:['hidden']});
    sync();
  }
  ...
  const start=()=>{setupCardValidation();setupSettingsEntry()};
