(()=>{
  ...
  function card(order){
    ...
    const actionClasses=`fs-order-actions${action?' has-primary':''}${needsCash?' has-cash':''}`;
    return `<article class="fs-order-card status-${esc(order.status)}" data-fs-order="${esc(order.id)}"><div class="fs-order-card-top"><div class="fs-order-primary"><div class="fs-order-number"><strong>#${esc(order.codigo||order.numero||order.id)}</strong><span class="status ${FSOrderStatus.css(order.status)}">${esc(FSOrderStatus.label(order.status,order.tipo))}</span></div><span class="fs-order-reference">${esc(reference(order))}</span><small class="fs-order-subtitle">${esc(subtitle(order))}</small></div><div class="fs-order-total"><b>${money(order.total)}</b>${payment}</div></div><div class="fs-order-card-middle"><div class="fs-order-meta"><span>${esc(FSOrderStatus.typeLabel(order.tipo))}</span><span>${new Date(order.created_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</span><span>${elapsed(order.created_at)}</span><span>${itemCount} ${itemCount===1?'item':'itens'}</span></div></div><div class="fs-order-items">${(order.itens_pedido||[]).slice(0,4).map(item=>`<span class="fs-order-item-chip">${Number(item.quantidade)}x ${esc(item.nome_produto)}</span>`).join('')}${(order.itens_pedido||[]).length>4?`<span class="fs-order-item-chip">+${order.itens_pedido.length-4}</span>`:''}</div>${order.status==='servido'&&saldo>0?`<div class="fs-order-payment-warning">Mesa servida e ainda ocupada. Saldo pendente: ${money(saldo)}.</div>`:''}<div class="fs-order-card-foot"><div class="${actionClasses}"><button class="btn btn-secondary" type="button" data-fs-detail="${esc(order.id)}">Detalhes</button>${needsCash?'<button class="btn btn-secondary" type="button" data-fs-cash>Abrir caixa</button>':''}${action?`<button class="btn btn-primary" type="button" data-fs-action="${esc(order.id)}">${esc(action.label)}</button>`:''}</div></div></article>`;
  }
