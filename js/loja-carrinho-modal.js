(() => {
  ...
  function openCheckoutForm(event) {
    ...
    if (typeof table !== 'undefined' && !table && typeof subtotal === 'function' && subtotal() < Number(settings.pedido_minimo)) return typeof setFeedback === 'function' && setFeedback(`O pedido mínimo é ${money(settings.pedido_minimo)}.`, 'error');
    if (typeof updateTotal === 'function') updateTotal();
    if (typeof open === 'function') open('checkout-modal');
    else byId('checkout-modal')?.classList.add('open');
    requestAnimationFrame(() => {
      syncCheckout();
      (currentType() === 'delivery' ? byId('customer-cep') : byId('customer-name'))?.focus({preventScroll:true});
    });
  }
  ...
  function installPaymentAdapter() {
    ...
    window.FSDeliveryPayment = Object.freeze({
      version: 1,
      getContext: () => ({orderType:currentType(),method:byId('payment-method')?.value||'',customer:{name:byId('customer-name')?.value.trim()||'',phone:byId('customer-phone')?.value.replace(/\D/g,'')||''},address:composeAddress(),cep:byId('customer-cep')?.value.replace(/\D/g,'')||'',totals:{subtotal:subtotal(),deliveryFee:fee(),total:total()}}),
  ...
  function fieldByLabel(labelText) {
    ...
    return [...document.querySelectorAll('#address-field label')].find(label => label.textContent.trim().toLowerCase() === normalized)?.control || null;
  }

  function addressInputs() {
    return {
      street: byId('customer-street') || byId('address-street') || byId('street') || fieldByLabel('Rua'),
      number: byId('customer-number') || byId('address-number') || byId('number') || fieldByLabel('Número'),
      neighborhood: byId('customer-neighborhood') || byId('address-neighborhood') || byId('neighborhood') || fieldByLabel('Bairro'),
      complement: byId('customer-complement') || byId('address-complement') || byId('complement') || fieldByLabel('Complemento ou referência'),
  ...
  function composeAddress() {
    const fields = addressInputs();
    const parts = [
      [fields.street?.value, fields.number?.value].filter(Boolean).join(', '),
      fields.neighborhood?.value,
      fields.complement?.value,
      fields.city?.value && fields.state?.value ? `${fields.city.value} - ${fields.state.value}` : fields.city?.value || fields.state?.value
    ].filter(Boolean);
    const value = parts.join(', ');
    if (fields.legacy && fields.legacy !== fields.street) fields.legacy.value = value;
    return value;
  }

  function selectMatchingRegion(neighborhood) {
    const select = byId('delivery-region');
    if (!select || !neighborhood) return;
    const normalize = value => String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
    const target = normalize(neighborhood);
    const option = [...select.options].find(item => normalize(item.textContent.split('•')[0]) === target || normalize(item.textContent).includes(target));
    if (!option) return;
  ...
  async function lookupCep(cep) {
    ...
    if (!input || input.dataset.lastLookup === cep) return;
    ...
    try {
      const response = await fetch(`https://viacep.com.br/ws/${cep}/json/`, {signal:requestController.signal,headers:{Accept:'application/json'}});
      if (!response.ok) throw new Error('Falha ao consultar o CEP');
      const data = await response.json();
      if (data.erro) {
        input.dataset.lastLookup = '';
        setCepStatus('CEP não encontrado. Confira o número ou preencha manualmente.', 'error');
        return;
      }
      const fields = addressInputs();
      setInputValue(fields.street, data.logradouro || '');
      setInputValue(fields.neighborhood, data.bairro || '');
      setInputValue(fields.city, data.localidade || '');
      setInputValue(fields.state, data.uf || '');
      if (fields.legacy && fields.legacy === fields.street) {
        ...
        setInputValue(fields.legacy, parts.join(', '));
      } else composeAddress();
      selectMatchingRegion(data.bairro);
      setCepStatus('Endereço localizado. Complete com número e complemento.', 'success');
      requestAnimationFrame(() => fields.number?.focus({preventScroll:true}));
    } catch (error) {
  ...
  function installCepField() {
    ...
    field.id = 'cep-field';
    field.innerHTML = `<label for="customer-cep">CEP</label><input id="customer-cep" name="cep" inputmode="numeric" autocomplete="postal-code" maxlength="9" placeholder="00000-000"><small id="cep-status" aria-live="polite">Digite o CEP para preencher o endereço automaticamente.</small>`;
    ...
    cep.addEventListener('input', event => {
      ...
      if (digits.length < 8) {
        requestController?.abort();
        event.target.dataset.lastLookup = '';
  ...
  function bindAddressComposition() {
    const fields = addressInputs();
    [fields.street,fields.number,fields.neighborhood,fields.complement,fields.city,fields.state].filter(Boolean).forEach(input => {
      if (input.dataset.addressComposeBound) return;
      input.dataset.addressComposeBound = 'true';
      input.addEventListener('input', composeAddress);
      input.addEventListener('change', composeAddress);
    });
  }

  function persistCepAndValidate() {
    const form = byId('checkout-form');
    if (!form || form.dataset.cepBound) return;
    form.dataset.cepBound = 'true';
    const storageKey = `fsdelivery_cep_${params.get('loja') || 'public'}`;
    const cep = byId('customer-cep');
    if (cep) cep.value = formatCep(localStorage.getItem(storageKey) || '');
    form.addEventListener('submit', event => {
      if (currentType() !== 'delivery') return;
      const digits = byId('customer-cep')?.value.replace(/\D/g, '') || '';
      if (digits.length !== 8) {
        event.preventDefault();
        event.stopImmediatePropagation();
        setCepStatus('Informe um CEP válido com 8 números.', 'error');
        byId('customer-cep')?.focus();
        return;
      }
      composeAddress();
      localStorage.setItem(storageKey, byId('customer-cep').value);
    }, true);
  }
