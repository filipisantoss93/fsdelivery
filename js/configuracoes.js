...
async function saveOperational(patch,message){
  const payload={...operational,...patch,estabelecimento_id:store.id,updated_at:new Date().toISOString()};
  const {data,error}=await db.from('configuracoes_operacionais').upsert(payload,{onConflict:'estabelecimento_id'}).select().single();
  if(error)return alert(error.message);operational=data;fillStore();if(message)alert(message);
}

async function saveDelivery(){
  const min=Math.max(0,Number(byId('delivery-min-config').value)||0),max=Math.max(min,Number(byId('delivery-max-config').value)||min);
  ...
  const {data,error}=await db.from('estabelecimentos').update(payload).eq('id',store.id).select().single();if(error)return alert(error.message);store=data;alert('Configurações de entrega salvas.');
}

async function saveRestaurant(){
  const payload={nome:byId('restaurant-name').value.trim(),telefone:byId('restaurant-phone').value.trim(),categoria:byId('restaurant-category').value,descricao:byId('restaurant-description').value.trim(),logo_url:byId('restaurant-logo').value.trim()||null,banner_url:byId('restaurant-banner').value.trim()||null,aberto:byId('restaurant-open').checked};
  if(!payload.nome)return alert('Informe o nome do estabelecimento.');const {data,error}=await db.from('estabelecimentos').update(payload).eq('id',store.id).select().single();if(error)return alert(error.message);store=data;fillStore();alert('Dados do restaurante salvos.');
}

function renderHours(){
  const map=new Map(hours.map(item=>[Number(item.dia_semana),item]));
  byId('hours-list').innerHTML=days.map((name,index)=>{const h=map.get(index)||{dia_semana:index,aberto:true,abertura:'08:00:00',fechamento:'22:00:00'};return `<div class="hours-row" data-day="${index}"><label class="switch"><input class="day-open" type="checkbox" ${h.aberto?'checked':''}><span></span></label><b>${name}</b><input class="day-start" type="time" value="${String(h.abertura||'08:00').slice(0,5)}"><span>até</span><input class="day-end" type="time" value="${String(h.fechamento||'22:00').slice(0,5)}"></div>`}).join('');
}
async function saveHours(){
  const payload=[...document.querySelectorAll('.hours-row')].map(row=>({estabelecimento_id:store.id,dia_semana:Number(row.dataset.day),aberto:row.querySelector('.day-open').checked,abertura:row.querySelector('.day-start').value||null,fechamento:row.querySelector('.day-end').value||null}));
  const {data,error}=await db.from('horarios_funcionamento').upsert(payload,{onConflict:'estabelecimento_id,dia_semana'}).select();if(error)return alert(error.message);hours=data||payload;alert('Horários salvos.');
}

function renderPayments(){
  const selected=new Set(Array.isArray(operational.formas_pagamento)?operational.formas_pagamento:paymentCatalog.slice(0,4));
  byId('payment-options').innerHTML=paymentCatalog.map(item=>`<label class="option-card"><input type="checkbox" value="${item}" ${selected.has(item)?'checked':''}><span><b>${item}</b><small>Disponível no checkout</small></span></label>`).join('');
}
...
async function saveNotifications(){
  if(byId('browser-notifications').checked&&'Notification' in window&&Notification.permission==='default')await Notification.requestPermission();
  await saveOperational({som_novo_pedido:byId('sound-orders').checked,notificacao_navegador:byId('browser-notifications').checked},'Notificações salvas.');
}
